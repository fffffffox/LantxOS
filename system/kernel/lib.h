...
////add to entry behind
inline void listAddToBehind(struct list *entry, struct list *new)  {
	new->next = entry->next;
	new->prev = entry;
	new->next->prev = new;
	entry->next = new;
...
////add to entry behind
inline void listAddToBefore(struct list *entry, struct list *new)  {
	new->next = entry;
	entry->prev->next = new;
	new->prev = entry->prev;
	entry->prev = new;
...
inline void listDel(struct list *entry) {
	entry->next->prev = entry->prev;
	entry->prev->next = entry->next;
}

inline long listIsEmpty(struct list *entry) {
